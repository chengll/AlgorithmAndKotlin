...
具体操作：
  ...
  三叉链表 ：便于找到前驱，即找到双亲；
   ...
   前序遍历的规则，只要经过该结点就访问，中序遍历是第二次经过该结点时访问；后序是第三次经过该结点时访问；
   重要案例，根据前序和中序序列来还原二叉树 和 已知 中序和后序序列来还原二叉树
   eg1:已知：前序序列为 A B C D E F G H I J
           中序序列为：C D B F E A I H G J
           由于前序可知A 为根结点， 所有可以知道  CDBEF  为A的左子树，而 IHGJ  为A 的右子树；
           由先序序列可知 B为根， CD 为B的左子树，EF 为B的右子树，由先序序列可知 C 为根结点，由中序知道，
           D在C的右侧，所以D 是C的右孩子，到这里B 的左子树已经确定完成，接着 看B 的右子树的序列，
           由先序可知 E为根，有中序知道 F在E 的左侧，所以F 是E 的左孩子；好到这里 A根的左孩子已经确定，
           接着看A 的右子树，由先序可知 G为根结点，IH 为G 的左孩子，J 为G 的右孩子， 由先序序列可知
           H为根，由中序序列可知 I 为H的左孩子 ，所以A 的左子树也完成，整儿二叉树也就构建完成了。

           最终完成的二叉树请查看 drawable /preandmid.jpg 图

     eg2:已知：中序序列为； B D C E A F H G
              后序序列为： D E C B H G F A
           由后序序列可知道 A 为根结点， 所以A 的左子树序列为  BDCE  ，右子树序列为 FHG
           由后序序列可知 B为根， 由中序可知 DCE 均是 B 的右子树， 由后序序列可知 C 为根结点，
           由中序 序列可知 D为C 的左孩子  E为C的右孩子，所以A的左边子树确立完成，接着看A 的右子树，
           由后序序列可知 F 为根结点，由中序可知 HG 均为 F的右孩子， 由后序可知 G 为根，由中序可知 H为 G 的左孩子
           到这里二叉树的结点关系已经特别清晰了。
